...
describe("nodes api moveNode", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });

  it("posts move request and returns node", async () => {
    const node = {
      id: "node-123",
      type: "FOLDER",
      name: "Moved folder",
    ...
    expect(url).toBe("http://localhost:1234/nodes/node-123/move");
  ...
  it("maps 409 to err.conflict", async () => {
    ...
    await expect(
      nodesApi.moveNode({ nodeId: "node-123", destinationParentId: "parent-2" }),
