...
if 'path="favorites" element={<FilesPage routeMode="favorites" />}' not in router_compact:
    print("route-favorites-not-files")
    raise SystemExit(1)

required_tokens = [
    'type RouteMode = "files" | "recent" | "favorites"',
    'isFavoritesRoute = routeMode === "favorites"',
    't("nav.favorites")',
    'msg.emptyFavorites',
    'listChildrenSort = isRecentRoute || isFavoritesRoute ? "updated_at" : "name"',
]
...
update_summary "PASS" "PASS" "true" "favorites route uses FilesPage and file-page regression checks pass"
echo "[$TASK] PASS"
