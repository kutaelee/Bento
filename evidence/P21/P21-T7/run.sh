...
if 'path="media"element={<FilesPagerouteMode="media"/>}' not in compact_router:
    print("route-media-not-files")
    raise SystemExit(1)

required_tokens = [
    'type RouteMode = "files" | "recent" | "favorites" | "shared" | "media"',
    'isMediaRoute = routeMode === "media"',
    't("nav.media")',
    'msg.emptyFolder',
]
...
update_summary "PASS" "PASS" "true" "media route mapped to FilesPage and FilesPage spec passes"
echo "[$TASK] PASS"
