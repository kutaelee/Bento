...
if ! pnpm -C "$ROOT/packages/ui" run lint >/tmp/p24t3-ui-lint.out 2>&1; then
  fail "ui lint failed: $(tail -n 1 /tmp/p24t3-ui-lint.out)"
fi

if ! pnpm -C "$ROOT/packages/ui" run test:visual >/tmp/p24t3-visual.out 2>&1; then
  fail "visual check failed: $(tail -n 1 /tmp/p24t3-visual.out)"
fi

pass_msg "ui lint + visual PASS"
