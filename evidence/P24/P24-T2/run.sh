...
if ! pnpm -C "$ROOT/packages/ui" run test:visual >/tmp/p24t2-visual.out 2>&1; then
  fail "visual check failed: $(tail -n 1 /tmp/p24t2-visual.out)"
fi

pass_msg "visual baseline mapping PASS"
