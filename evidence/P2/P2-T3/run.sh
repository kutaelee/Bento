...
EVID_DIR="$ROOT_DIR/evidence/P2/P2-T3"
...
REFRESH_TOKEN="$(jq -r '.tokens.refresh_token' "$RAW_LOGIN")"
...
# 1) logout success
curl -sS -o "$ACT_HTTP/logout_001.body.json" -w "%{http_code}" \
  -H "authorization: Bearer ${ACCESS_TOKEN}" \
  -H 'content-type: application/json' \
  -d '{}' \
  "http://localhost:${PORT}/auth/logout" \
  >"$ACT_HTTP/logout_001.status.txt"

STATUS_1="$(cat "$ACT_HTTP/logout_001.status.txt")"
if [[ "$STATUS_1" != "200" ]]; then
  echo "expected status 200, got $STATUS_1" >"$ACT_HTTP/logout_001.assert.txt"
  cat "$ACT_HTTP/logout_001.body.json" >>"$ACT_HTTP/logout_001.assert.txt" || true
  exit 1
fi

jq -e '.ok==true' "$ACT_HTTP/logout_001.body.json" >"$ACT_HTTP/logout_001.assert.txt" 2>&1

# 2) refresh should fail after logout
curl -sS -o "$ACT_HTTP/refresh_after_logout_001.body.json" -w "%{http_code}" \
  -H 'content-type: application/json' \
  -d "{\"refresh_token\":\"${REFRESH_TOKEN}\"}" \
  "http://localhost:${PORT}/auth/refresh" \
  >"$ACT_HTTP/refresh_after_logout_001.status.txt"

STATUS_2="$(cat "$ACT_HTTP/refresh_after_logout_001.status.txt")"
if [[ "$STATUS_2" != "401" ]]; then
  echo "expected status 401, got $STATUS_2" >"$ACT_HTTP/refresh_after_logout_001.assert.txt"
  cat "$ACT_HTTP/refresh_after_logout_001.body.json" >>"$ACT_HTTP/refresh_after_logout_001.assert.txt" || true
  exit 1
fi

jq -e '.error.code and .error.message' "$ACT_HTTP/refresh_after_logout_001.body.json" >"$ACT_HTTP/refresh_after_logout_001.assert.txt" 2>&1
