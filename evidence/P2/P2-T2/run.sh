...
EVID_DIR="$ROOT_DIR/evidence/P2/P2-T2"
...
OLD_REFRESH_TOKEN="$(jq -r '.tokens.refresh_token' "$RAW_LOGIN")"
if [[ -z "$OLD_REFRESH_TOKEN" || "$OLD_REFRESH_TOKEN" == "null" ]]; then
  echo "missing tokens.refresh_token" >"$ACT_HTTP/login.assert.txt"
  exit 1
fi

# 1) refresh success
RAW_REFRESH_1="$ACT_HTTP/refresh_001.body.raw.json"

curl -sS -o "$RAW_REFRESH_1" -w "%{http_code}" \
  -H 'content-type: application/json' \
  -d "{\"refresh_token\":\"${OLD_REFRESH_TOKEN}\"}" \
  "http://localhost:${PORT}/auth/refresh" \
  >"$ACT_HTTP/refresh_001.status.txt"

STATUS_1="$(cat "$ACT_HTTP/refresh_001.status.txt")"
if [[ "$STATUS_1" != "200" ]]; then
  echo "expected status 200, got $STATUS_1" >"$ACT_HTTP/refresh_001.assert.txt"
  cat "$RAW_REFRESH_1" >>"$ACT_HTTP/refresh_001.assert.txt" || true
  exit 1
fi

jq -e '.token_type=="Bearer" and (.access_token|type=="string") and (.refresh_token|type=="string") and (.expires_in_seconds|type=="number")' \
  "$RAW_REFRESH_1" >"$ACT_HTTP/refresh_001.assert.txt" 2>&1

jq \
  '(.access_token)="REDACTED" |
   (.refresh_token)="REDACTED"' \
  "$RAW_REFRESH_1" >"$ACT_HTTP/refresh_001.body.json"
rm -f "$RAW_REFRESH_1"

# 2) reuse old refresh token should fail
curl -sS -o "$ACT_HTTP/refresh_002.body.json" -w "%{http_code}" \
  -H 'content-type: application/json' \
  -d "{\"refresh_token\":\"${OLD_REFRESH_TOKEN}\"}" \
  "http://localhost:${PORT}/auth/refresh" \
  >"$ACT_HTTP/refresh_002.status.txt"

STATUS_2="$(cat "$ACT_HTTP/refresh_002.status.txt")"
if [[ "$STATUS_2" != "401" ]]; then
  echo "expected status 401, got $STATUS_2" >"$ACT_HTTP/refresh_002.assert.txt"
  exit 1
fi

jq -e '.error.code and .error.message' "$ACT_HTTP/refresh_002.body.json" >"$ACT_HTTP/refresh_002.assert.txt" 2>&1

# Redact login output (not strictly needed for this task)
jq \
  '(.user.id)="REDACTED" |
   (.user.created_at)="REDACTED" |
   (.user.last_login_at)="REDACTED" |
   (.tokens.access_token)="REDACTED" |
   (.tokens.refresh_token)="REDACTED"' \
  "$RAW_LOGIN" >"$ACT_HTTP/login.body.json"
rm -f "$RAW_LOGIN"
