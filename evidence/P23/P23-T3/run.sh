...
TASK_FILE="$ROOT/packages/ui/src/app/AdminStoragePage.tsx"
TASK_STYLES="$ROOT/packages/ui/src/app/AdminStoragePage.css"
...
if ! grep -q "admin.storage" "$ROUTE_FILE"; then
  fail "admin storage route not wired"
fi
...
pnpm -C packages/ui exec eslint src/app/AdminStoragePage.tsx src/app/AdminRoutes.tsx >/tmp/${TASK}-eslint.log 2>&1 || {
...
update_summary "PASS" "PASS" "true" "admin storage page refactor implemented and lint/typecheck passed"
echo "[$TASK] PASS"
