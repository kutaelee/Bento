#!/usr/bin/env bash
set -euo pipefail

fail() {
  echo "[P5-T1] $1" >&2
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
EVID_DIR="$ROOT_DIR/evidence/P5/P5-T1"
ACT_HTTP="$EVID_DIR/actual/http"
ACT_LOGS="$EVID_DIR/actual/logs"

mkdir -p "$ACT_HTTP" "$ACT_LOGS"
cd "$ROOT_DIR"

if [[ -z "${EVIDENCE_REUSE_DB:-}" ]]; then docker compose -f compose.yaml down -v >/dev/null 2>&1 || true; fi
docker compose -f compose.yaml up -d postgres

for _ in {1..60}; do
  if docker compose -f compose.yaml exec -T postgres pg_isready -U nimbus -d ${NIMBUS_DB:-nimbus_drive} >/dev/null 2>&1; then
    break
  fi
  sleep 1
done

PORT="$(python3 - <<'PY'
import socket
s = socket.socket()
s.bind(('',0))
print(s.getsockname()[1])
s.close()
PY
)"
(
  cd "$ROOT_DIR"
  PORT="$PORT" node scripts/dev_server.mjs >"$ACT_LOGS/server.log" 2>&1 &
  echo $! >"$ACT_LOGS/server.pid"
)

cleanup() {
  if [[ -f "$ACT_LOGS/server.pid" ]]; then
    PID="$(cat "$ACT_LOGS/server.pid" || true)"
    if [[ -n "${PID:-}" ]] && kill -0 "$PID" 2>/dev/null; then
      kill "$PID" 2>/dev/null || true
      sleep 0.2 || true
      kill -9 "$PID" 2>/dev/null || true
    fi
  fi
  if [[ -z "${EVIDENCE_REUSE_DB:-}" ]]; then docker compose -f compose.yaml down -v >/dev/null 2>&1 || true; fi
}
trap cleanup EXIT

ready=""
for _ in {1..300}; do
  if curl -sSf "http://localhost:${PORT}/health" >/dev/null 2>&1; then
    ready="yes"
    break
  fi
  sleep 0.2
done
if [[ -z "$ready" ]]; then
  fail "server did not become ready in time"
  exit 1
fi

if ! command -v jq >/dev/null 2>&1; then
  fail "jq is required"
  exit 1
fi

RAW_SETUP="$ACT_HTTP/setup_admin.body.raw.json"

# CI flake guard: dev_server may report /health OK before setup/admin is ready to accept writes.
# Retry a few times to avoid transient 5xx/000 during initial boot.
setup_status=""
for _ in {1..20}; do
  curl -sS -o "$RAW_SETUP" -w "%{http_code}" \
    -H 'content-type: application/json' \
    -d '{"username":"admin","password":"CorrectHorseBatteryStaple","display_name":"Admin","locale":"en-US"}' \
    "http://localhost:${PORT}/setup/admin" \
    >"$ACT_HTTP/setup_admin.status.txt" || true
  setup_status="$(cat "$ACT_HTTP/setup_admin.status.txt" || true)"
  if [[ "$setup_status" == "201" ]]; then
    break
  fi
  sleep 0.2
done

if [[ "$setup_status" != "201" ]]; then
  fail "expected setup/admin 201 (got ${setup_status:-empty})"
  exit 1
fi

ACCESS_TOKEN="$(jq -r '.tokens.access_token' <"$RAW_SETUP")"
if [[ -z "$ACCESS_TOKEN" || "$ACCESS_TOKEN" == "null" ]]; then
  fail "missing access token from setup/admin"
  exit 1
fi

ROOT_ID="00000000-0000-0000-0000-000000000001"

RAW_PARENT="$ACT_HTTP/create_parent.body.raw.json"
curl -sS -o "$RAW_PARENT" -w "%{http_code}" \
  -H 'content-type: application/json' \
  -H "authorization: Bearer ${ACCESS_TOKEN}" \
  -d '{"parent_id":"'"$ROOT_ID"'","name":"Uploads"}' \
  "http://localhost:${PORT}/nodes/folders" \
  >"$ACT_HTTP/create_parent.status.txt" || true
if [[ "$(cat "$ACT_HTTP/create_parent.status.txt" || true)" != "201" ]]; then
  fail "expected parent create 201"
  exit 1
fi

PARENT_ID="$(jq -r '.id' <"$RAW_PARENT")"

RAW_UPLOAD="$ACT_HTTP/create_upload.body.raw.json"
curl -sS -o "$RAW_UPLOAD" -w "%{http_code}" \
  -H 'content-type: application/json' \
  -H "authorization: Bearer ${ACCESS_TOKEN}" \
  -d '{"parent_id":"'"$PARENT_ID"'","filename":"hello.txt","size_bytes":12,"sha256":null,"mime_type":"text/plain","modified_at":null}' \
  "http://localhost:${PORT}/uploads" \
  >"$ACT_HTTP/create_upload.status.txt" || true

if [[ "$(cat "$ACT_HTTP/create_upload.status.txt" || true)" != "201" ]]; then
  fail "expected uploads create 201"
  exit 1
fi

jq -e '.upload_id | test("^[0-9a-fA-F-]{36}$")' "$RAW_UPLOAD" >"$ACT_HTTP/create_upload.upload_id.assert.txt"
jq -e '.status and (.chunk_size_bytes|type=="number" and . >= 1048576) and (.total_chunks|type=="number" and . >= 1) and (.dedup_hit==false or .dedup_hit==true)' "$RAW_UPLOAD" >"$ACT_HTTP/create_upload.fields.assert.txt"

cat > "$EVID_DIR/summary.json" <<'JSON'
{
  "piece_id": "P5",
  "task_id": "P5-T1",
  "result": "PASS",
  "pass": true,
  "checks": [
    {"name": "create_upload_201", "expected": "POST /uploads returns 201", "actual_path": "evidence/P5/P5-T1/actual/http/create_upload.status.txt", "pass": true},
    {"name": "create_upload_fields", "expected": "CreateUploadResponse contains upload_id/status/chunk_size_bytes/total_chunks/dedup_hit", "actual_path": "evidence/P5/P5-T1/actual/http/create_upload.fields.assert.txt", "pass": true}
  ],
  "artifacts": {
    "cases": [
      "evidence/P5/P5-T1/cases/P5-T1-UPLOADS-CREATE-001.case.yaml"
    ],
    "run_sh": "evidence/P5/P5-T1/run.sh",
    "expected_md": "evidence/P5/P5-T1/expected.md",
    "actual": {
      "http": {
        "setup": "evidence/P5/P5-T1/actual/http/setup_admin.status.txt",
        "parent": "evidence/P5/P5-T1/actual/http/create_parent.status.txt",
        "upload": "evidence/P5/P5-T1/actual/http/create_upload.status.txt"
      },
      "logs": {
        "server": "evidence/P5/P5-T1/actual/logs/server.log"
      }
    }
  }
}
JSON
